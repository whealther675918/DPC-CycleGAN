...
from .create_light import MutiScaleLuminanceEstimation
import numpy as np
from .frequency import HighPassFilter, LowPassFilter
...
class Generator(nn.Module):
    def __init__(self, device):
        ...
        self.depth = DepthEstimationNet()
        self.light = MutiScaleLuminanceEstimation()
        self.high = HighPassFilter()
        self.low = LowPassFilter()
    ...
    def forward(self, underwater, clean):
        # 生成水下图像
        # light = self.light(underwater, self.device)
        # depth = self.depth(clean)

        # 水下图像增强
        light = self.light(clean, self.device)
        depth = self.depth(underwater)

        # print(light.shape, depth.shape, clean.shape)
        x = torch.cat([light, depth, underwater], 1)
        ...
        outlight = self.light(out, self.device)
        outdepth = self.depth(out)

        outhigh = self.high(out)
        outhigh = transforms.functional.to_tensor(outhigh)
        outhigh = outhigh.unsqueeze(0)

        outlow = self.low(out)
        outlow = transforms.functional.to_tensor(outlow)
        outlow = outlow.unsqueeze(0)

        highB = self.high(underwater)
        highB = transforms.functional.to_tensor(highB)
        highB = highB.unsqueeze(0)

        lowB = self.low(underwater)
        lowB = transforms.functional.to_tensor(lowB)
        lowB = lowB.unsqueeze(0)


        # print(out.shape)
        # return out, depth, light, outlight, outdepth, outhigh, outlow, highB, lowB
        return out
